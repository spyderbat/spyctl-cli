...
@click.command("machines", cls=lib.CustomCommand, epilog=lib.SUB_EPILOG)
...
def handle_get_machines(name_or_id, output, st, et, **filters):
    ...
    if output in [lib.OUTPUT_DEFAULT, lib.OUTPUT_WIDE]:
        summary = _r.machines.machines_summary_output(ctx, sources, (st, et), pipeline)
        cli.show(summary, lib.OUTPUT_RAW)
    else:
        for machine in sq_api.get_machines(
            *ctx.get_api_data(),
            sources,
            (st, et),
            pipeline=pipeline,
